...
Build on the node that hosts `luna-brain` so kubelet can use `imagePullPolicy: Never`:

```bash
ssh aachten@raspberrypi3 '
  cd /home/aachten/homelab-app &&
  docker build -f brain/Dockerfile -t homelab-app-brain ./brain
'
```
...
```bash
kubectl -n apps set image deployment/luna-brain luna-brain=homelab-app-brain
kubectl -n apps rollout status deployment/luna-brain --timeout=180s
```
